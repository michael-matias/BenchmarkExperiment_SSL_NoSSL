...
app.post('/service3', (req, res) => {
    if (req.body.jwtData) {
        res.send('OK');  // This assumes token came raw and service3 decoded it.
...
app.use((err, req, res, next) => {
    console.error("Error in service3:", err.message);
    res.status(500).send("Failed in service3");
});

// HTTP Server
app.listen(PORT_HTTP, () => {
    console.log(`Service 3 HTTP started on port ${PORT_HTTP}`);
});
...
try {
    ...
    console.error('Error loading HTTPS options for Service 3:', error.message);
...
https.createServer(httpsOptions, app).listen(PORT_HTTPS, () => {
    console.log(`Service 3 HTTPS started on port ${PORT_HTTPS}`);
});
